/****************************************************************************
** Meta object code from reading C++ file 'stepper.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.15.10)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include <memory>
#include "../../../App/ui/stepper/stepper.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'stepper.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.15.10. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_stepper_t {
    QByteArrayData data[11];
    char stringdata0[77];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_stepper_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_stepper_t qt_meta_stringdata_stepper = {
    {
QT_MOC_LITERAL(0, 0, 7), // "stepper"
QT_MOC_LITERAL(1, 8, 4), // "step"
QT_MOC_LITERAL(2, 13, 0), // ""
QT_MOC_LITERAL(3, 14, 4), // "stop"
QT_MOC_LITERAL(4, 19, 3), // "run"
QT_MOC_LITERAL(5, 23, 8), // "setSpeed"
QT_MOC_LITERAL(6, 32, 5), // "value"
QT_MOC_LITERAL(7, 38, 8), // "stepSlot"
QT_MOC_LITERAL(8, 47, 8), // "stopSlot"
QT_MOC_LITERAL(9, 56, 7), // "runSlot"
QT_MOC_LITERAL(10, 64, 12) // "setSpeedSlot"

    },
    "stepper\0step\0\0stop\0run\0setSpeed\0value\0"
    "stepSlot\0stopSlot\0runSlot\0setSpeedSlot"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_stepper[] = {

 // content:
       8,       // revision
       0,       // classname
       0,    0, // classinfo
       8,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       4,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   54,    2, 0x06 /* Public */,
       3,    0,   55,    2, 0x06 /* Public */,
       4,    0,   56,    2, 0x06 /* Public */,
       5,    1,   57,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       7,    0,   60,    2, 0x08 /* Private */,
       8,    0,   61,    2, 0x08 /* Private */,
       9,    0,   62,    2, 0x08 /* Private */,
      10,    1,   63,    2, 0x08 /* Private */,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void, QMetaType::Int,    6,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void, QMetaType::Int,    6,

       0        // eod
};

void stepper::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        auto *_t = static_cast<stepper *>(_o);
        (void)_t;
        switch (_id) {
        case 0: _t->step(); break;
        case 1: _t->stop(); break;
        case 2: _t->run(); break;
        case 3: _t->setSpeed((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 4: _t->stepSlot(); break;
        case 5: _t->stopSlot(); break;
        case 6: _t->runSlot(); break;
        case 7: _t->setSpeedSlot((*reinterpret_cast< int(*)>(_a[1]))); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        {
            using _t = void (stepper::*)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&stepper::step)) {
                *result = 0;
                return;
            }
        }
        {
            using _t = void (stepper::*)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&stepper::stop)) {
                *result = 1;
                return;
            }
        }
        {
            using _t = void (stepper::*)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&stepper::run)) {
                *result = 2;
                return;
            }
        }
        {
            using _t = void (stepper::*)(int );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&stepper::setSpeed)) {
                *result = 3;
                return;
            }
        }
    }
}

QT_INIT_METAOBJECT const QMetaObject stepper::staticMetaObject = { {
    QMetaObject::SuperData::link<QWidget::staticMetaObject>(),
    qt_meta_stringdata_stepper.data,
    qt_meta_data_stepper,
    qt_static_metacall,
    nullptr,
    nullptr
} };


const QMetaObject *stepper::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *stepper::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_stepper.stringdata0))
        return static_cast<void*>(this);
    return QWidget::qt_metacast(_clname);
}

int stepper::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 8)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 8;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 8)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 8;
    }
    return _id;
}

// SIGNAL 0
void stepper::step()
{
    QMetaObject::activate(this, &staticMetaObject, 0, nullptr);
}

// SIGNAL 1
void stepper::stop()
{
    QMetaObject::activate(this, &staticMetaObject, 1, nullptr);
}

// SIGNAL 2
void stepper::run()
{
    QMetaObject::activate(this, &staticMetaObject, 2, nullptr);
}

// SIGNAL 3
void stepper::setSpeed(int _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t1))) };
    QMetaObject::activate(this, &staticMetaObject, 3, _a);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
